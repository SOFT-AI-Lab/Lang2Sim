...
def LDFT2D_MT_H(M, T):
    """compute water adsorption hysteresis of a hydrophilic porous matrix at different temperatures."""
    ...
    densities.append(tf.zeros(batch_size))


    grid = grids[0]
    dft_curve = np.float32(densities)
    relative_humidity = np.float32([i*STEP_SIZE*100 for i in range(N_ADSORP+1)] + [(1.0-(i-40)*STEP_SIZE)*100 for i in range(N_ADSORP+1,N_ITER+1)])
    ...
    ax = plt.subplot(312)
    ax.clear()
    ax.set_title('Adsorption hysteresis')
    ...
    water=np.array(rs)[74,0,1:-1,1:-1,0]
    ax = plt.subplot(3,4,9)
    ax.clear()
    ax.set_title('RH = 10% (Desorption)')
    cmap = plt.cm.Greys
    alphas = np.flip(1-grid,0)
    colors = Normalize(0, 1, clip=True)(np.flip(1-grid, 0))
    colors = cmap(colors)
    ...
    water=np.array(rs)[66,0,1:-1,1:-1,0]
    ax = plt.subplot(3,4,10)
    ax.clear()
    ax.set_title('RH = 30% (Desorption)')
    cmap = plt.cm.Greys
    alphas = np.flip(1-grid,0)
    colors = Normalize(0, 1, clip=True)(np.flip(1-grid, 0))
    colors = cmap(colors)
    ...
    water=np.array(rs)[58,0,1:-1,1:-1,0]
    ax = plt.subplot(3,4,11)
    ax.clear()
    ax.set_title('RH = 50% (Desorption)')
    cmap = plt.cm.Greys
    alphas = np.flip(1-grid,0)
    colors = Normalize(0, 1, clip=True)(np.flip(1-grid, 0))
    colors = cmap(colors)
    ...
    water=np.array(rs)[42,0,1:-1,1:-1,0]
    ax = plt.subplot(3,4,12)
    ax.clear()
    ax.set_title('RH = 90% (Desorption)')
    cmap = plt.cm.Greys
    alphas = np.flip(1-grid,0)
    colors = Normalize(0, 1, clip=True)(np.flip(1-grid, 0))
    colors = cmap(colors)
    ...
    plt.savefig("sorption_hysteresis.jpg")
